...
export const getRoomTypeByID = async (req: Request, res: Response)  : Promise<CustomResponse> =>{
    try{
        let {user_id , user_type_id} = (req as RequestCheckToken);
        if(user_type_id != `1`){
            return res.json({
        ...
        let result = await pool.request()
        .input('id', id)
        .execute('RoomType_getDetailByID_AdminWeb')
        ...
        if(!RoomTypeDetail){
            return res.json({
                ...
                type : false,
                data: {},
                message: "Lấy chi tiết loại phòng thất bại"
            });
        }
        return res.json({
            code: 200,
